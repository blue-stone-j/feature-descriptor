...
cv::Mat SiftMatch::LMS(const cv::Mat &match1_xy, const cv::Mat &match2_xy, std::string model, float &rmse)
{
  ...
  else if (model == std::string("perspective"))
  {
    ...
    cv::Mat A1; // 完成的 A 矩阵，(8,8)
    A1.create(2 * N, 8, CV_32FC1);
    A.copyTo(A1(cv::Range::all(), cv::Range(0, 6)));
    A2.copyTo(A1(cv::Range::all(), cv::Range(6, 8)));

    cv::Mat values;                      // values中存放的是待求的8个参数
    solve(A1, B, values, cv::DECOMP_QR); //(2N,8)*(8,1)=(2N,1)好像本身就有点超定矩阵的意思
    ...
    cv::Mat temp1 = cv::Mat::ones(1, N, CV_32FC1);
    cv::Mat temp2;
    temp2.create(3, N, CV_32FC1);

    match2_xy_trans.copyTo(temp2(cv::Range(0, 2), cv::Range::all()));
    temp1.copyTo(temp2(cv::Range(2, 3), cv::Range::all()));

    cv::Mat match2_xy_change    = change * temp2; // 待配准图像中的特征点在参考图中的映射结果
    cv::Mat match2_xy_change_12 = match2_xy_change(cv::Range(0, 2), cv::Range::all());
    ...
    cv::Mat diff = match2_xy_change_12 - match1_xy_trans;
    pow(diff, 2, diff);
  ...
  else if (model == std::string("similarity"))
  {
    ...
    cv::Vec4f values;
    solve(A3, B, values, cv::DECOMP_QR);
    change = (cv::Mat_<float>(3, 3) << values(0), values(1), values(2),
    ...
    cv::Mat diff             = match2_xy_change - match1_xy_trans; // 求差
    pow(diff, 2, diff);
    rmse = (float)sqrt(cv::sum(diff)(0) * 1.0) / N; // sum输出是各个通道的和
  }
...
cv::Mat SiftMatch::improve_LMS(const cv::Mat &match1_xy, const cv::Mat &match2_xy, std::string model, float &rmse)
{
  ...
  else if (model == std::string("perspective"))
  {
    ...
    cv::Mat temp1 = cv::Mat::ones(1, N, CV_32FC1);
    cv::Mat temp2; // 存放处理后的特征点(x,y,1)T
    temp2.create(3, N, CV_32FC1);
...
cv::Mat SiftMatch::ransac(const std::vector<cv::Point2f> &points_1, const std::vector<cv::Point2f> &points_2, std::string model, float threshold, std::vector<bool> &inliers, float &rmse)
{
  ...
  if (model == std::string("similarity"))
  {
    n             = 2;
  ...
  for (size_t i = 0; i < iterations; ++i) // 迭代次数
  {
    ...
    if (model == std::string("perspective"))
    {
      // 变换矩阵计算待配准图像特征点在参考图像中的映射点
      cv::Mat match2_xy_change = T * arr_2; // arr_2 中存放的是待配准图像的特征点 (3,N),

      // match2_xy_change(cv::Range(0, 2), cv::Range::all())意思是提取 match2_xy_change 的 0、1 行，所有的列
      cv::Mat match2_xy_change_12 = match2_xy_change(cv::Range(0, 2), cv::Range::all());
      ...
      cv::Mat diff = match2_xy_change_12 - arr_1(cv::Range(0, 2), cv::Range::all());

      pow(diff, 2, diff);

      // 第一行和第二行求和，即两点间距离的平方
      cv::Mat add = diff(cv::Range(0, 1), cv::Range::all()) + diff(cv::Range(1, 2), cv::Range::all());
    ...
    else if (model == std::string("affine") || model == std::string("similarity"))
    {
      ...
      cv::Mat diff             = match2_xy_change - arr_1;
  ...
  cv::Mat consensus_arr1, consensus_arr2; // 经过迭代后最终确认正确匹配的点
...
void SiftMatch::mosaic_map(const cv::Mat &image_1, const cv::Mat &image_2, cv::Mat &chessboard_1, cv::Mat &chessboard_2, cv::Mat &mosaic_image, int width)
{
  ...
  for (int i = 0; i < row_grids_1; i = i + 2)
  {
    ...
    {
      cv::Range range_x(j * width, (j + 1) * width);
      cv::Range range_y(i * width, (i + 1) * width);
 ...
 */
inline void median_filter(cv::Mat &image, const std::vector<std::vector<int>> &pos)
{
  ...
  switch (channels)
  {
    ...
    case 3: // 3通道
      for (auto beg = pos.cbegin(); beg != pos.cend(); ++beg)
      {
        int i              = (*beg)[0]; // y
        int j              = (*beg)[1]; // x
        cv::Vec3b &pix_val = image.at<cv::Vec3b>(i, j);
        ...
        for (int row = -1; row <= 1; ++row)
        {
          for (int col = -1; col <= 1; ++col)
          {
            if (i + row >= 0 && i + row < image.rows && j + col >= 0 && j + col < image.cols)
            {
              pixs.push_back(image.at<cv::Vec3b>(i + row, j + col));
            }
          }
        }
        ...
        // 排序
        std::sort(pixs.begin(), pixs.end(),
                  [pix_val](const cv::Vec3b &a, const cv::Vec3b &b) -> bool {
                    return cv::sum((a).ddot(a))[0] < cv::sum((b).ddot(b))[0];
                  });
        pix_val = pixs[pixs.size() / 2];
      }
      break;
...
void SiftMatch::image_fusion(const cv::Mat &image_1, const cv::Mat &image_2, const cv::Mat T, cv::Mat &fusion_image, cv::Mat &matched_image)
{
  ...
  // 创建一个（3，3）float 矩阵 cv::Mat_ 是一个模板类
  cv::Mat T_temp = (cv::Mat_<float>(3, 3) << 1, 0, cols_1, 0, 1, rows_1, 0, 0, 1);
  cv::Mat T_1    = T_temp * T;

  // 对参考图像和待配准图像进行变换
  cv::Mat trans_1, trans_2; // same type as image_2_temp

  trans_1 = cv::Mat::zeros(3 * rows_1, 3 * cols_1, image_1_temp.type());                      // 创建扩大后的矩阵
  image_1_temp.copyTo(trans_1(cv::Range(rows_1, 2 * rows_1), cv::Range(cols_1, 2 * cols_1))); // 把image_1_temp中的数据复制到扩大后矩阵的对应位置

  warpPerspective(image_2_temp, trans_2, T_1, cv::Size(3 * cols_1, 3 * rows_1), cv::INTER_LINEAR, 0, cv::Scalar::all(0));
   ...
   *const cv::Scalar& borderValue=cv::Scalar()：边界的颜色设置，一般默认是0，Scalar::all(0)对影像边界外进行填充*/

  // 使用简单的均值法进行图像融合
  cv::Mat trans = trans_2.clone(); // 把经过透视变换的image_2复制给trans
  int nRows     = rows_1;
  int nCols     = cols_1;
  int len       = nCols;
  bool flag_1   = false;
  ...
  std::vector<std::vector<int>> positions; // 保存边缘位置坐标
      ...
      for (int i = 0; i < nRows; ++i)
      {
        ...
        for (int j = 0; j < nCols; ++j)
        {
          ...
          else // 对于重合区域
          {
            ...
            ptr[j + len] = cv::saturate_cast<uchar>(((float)ptr[j + len] + (float)ptr_1[j + len]) / 2);
          }
        }
      }
      break;
    case 3:                                // 如果图像是三通道的
      ...
      for (int i = 0; i < nRows; ++i)
      {
        ...
        for (int j = 0; j < nCols; ++j)
        {
          ...
          if (ptr[nj + len] == 0 && ptr[nj + len + 1] == 0 && ptr[nj + len + 2] == 0 && ptr_1[nj + len] != 0 && ptr_1[nj + len + 1] != 0 && ptr_1[nj + len + 2] != 0)
          {
            flag_1 = true;
            if (flag_2) // 表明从重合区域过度到了非重合区域
            {
          ...
          { // 对于重合区域
            ...
            ptr[nj + len]     = cv::saturate_cast<uchar>(((float)ptr[nj + len] + (float)ptr_1[nj + len]) / 2);
            ptr[nj + len + 1] = cv::saturate_cast<uchar>(((float)ptr[nj + len + 1] + (float)ptr_1[nj + len + 1]) / 2);
            ptr[nj + len + 2] = cv::saturate_cast<uchar>(((float)ptr[nj + len + 2] + (float)ptr_1[nj + len + 2]) / 2);
          }
        }
      }
      break;
  ...
  // 删除多余的区域
  cv::Mat left_up    = T_1 * (cv::Mat_<float>(3, 1) << 0, 0, 1);                   // 左上角
  cv::Mat left_down  = T_1 * (cv::Mat_<float>(3, 1) << 0, rows_2 - 1, 1);          // 左下角
  cv::Mat right_up   = T_1 * (cv::Mat_<float>(3, 1) << cols_2 - 1, 0, 1);          // 右上角
  cv::Mat right_down = T_1 * (cv::Mat_<float>(3, 1) << cols_2 - 1, rows_2 - 1, 1); // 右下角
  ...
  // 计算x,y坐标的范围
  float temp_1 = cv::min(left_up.at<float>(0, 0), left_down.at<float>(0, 0));
  float temp_2 = cv::min(right_up.at<float>(0, 0), right_down.at<float>(0, 0));
  float min_x  = cv::min(temp_1, temp_2);

  temp_1      = cv::max(left_up.at<float>(0, 0), left_down.at<float>(0, 0));
  temp_2      = cv::max(right_up.at<float>(0, 0), right_down.at<float>(0, 0));
  float max_x = cv::max(temp_1, temp_2);

  temp_1      = cv::min(left_up.at<float>(1, 0), left_down.at<float>(1, 0));
  temp_2      = cv::min(right_up.at<float>(1, 0), right_down.at<float>(1, 0));
  float min_y = cv::min(temp_1, temp_2);

  temp_1      = cv::max(left_up.at<float>(1, 0), left_down.at<float>(1, 0));
  temp_2      = cv::max(right_up.at<float>(1, 0), right_down.at<float>(1, 0));
  float max_y = cv::max(temp_1, temp_2);

  int X_min = cv::max(cvFloor(min_x), 0);
  int X_max = cv::min(cvCeil(max_x), 3 * cols_1 - 1);
  int Y_min = cv::max(cvFloor(min_y), 0);
  ...
  cv::Range X_range(X_min, X_max + 1);

  fusion_image  = trans(Y_range, X_range);
  matched_image = trans_2(Y_range, X_range);

  cv::Mat ref_matched = trans_1(Y_range, X_range);

  // 生成棋盘网格图像
  /*cv::Mat chessboard_1, chessboard_2;
...
void SiftMatch::match_des(const cv::Mat &des_1, const cv::Mat &des_2, std::vector<std::vector<cv::DMatch>> &dmatchs, DIS_CRIT dis_crite)
{
  int num_des_1 = des_1.rows;
  int num_des_2 = des_2.rows;
  int dims_des  = des_1.cols;

  std::vector<cv::DMatch> match(2);
  ...
  else if (dis_crite == 1) // cos距离
  {
    ...
    double aa       = (double)cv::getTickCount();
    cv::Mat mul_des = des_1 * trans_des2;
    // gemm(des_1, des_2, 1, Mat(), 0, mul_des, GEMM_2_T);
    double time1 = ((double)cv::getTickCount() - aa) / cv::getTickFrequency();
    std::cout << "cos距离中矩阵乘法花费时间： " << time1 << "s" << std::endl;
...
/*******************建立尺度直方图、ROM 直方图************************/
void SiftMatch::scale_ROM_Histogram(const std::vector<cv::DMatch> &matches, float *scale_hist, float *ROM_hist, int n)
{
                         ...
                         std::vector<cv::KeyPoint> keys_2, std::string model, std::vector<cv::DMatch> &right_matchs, cv::Mat &matched_line, std::vector<cv::DMatch> &init_matchs)
{
  ...
  std::cout << "距离比之后初始匹配点对个数是： " << init_matchs.size() << std::endl;
  ...
  // 提取出处正确匹配点对
  int i = 0;
  std::vector<cv::Point2f> point_11, point_22;
  std::vector<cv::DMatch>::iterator itt = init_matchs.begin();
  for (std::vector<bool>::iterator it = inliers.begin(); it != inliers.end(); ++it, ++itt)
  {
    if (*it) // 如果是正确匹配点对
    {
  ...
  std::cout << "使用RANSAC删除错误点对,且返回正确匹配个数： " << right_matchs.size() << std::endl;
  std::cout << "误差rmse: " << rmse << std::endl;
  ...
  // 保存和显示检测到的特征点
  cv::Mat keys_image_1, keys_image_2;                                  // 输出矩阵，类似画布
  drawKeypoints(image_1, keys_1, keys_image_1, cv::Scalar(0, 255, 0)); // 该函数用于绘制图像中的特征点
  drawKeypoints(image_2, keys_2, keys_image_2, cv::Scalar(0, 255, 0));
