...
/*pyramid表示高斯金字塔或者DOG金字塔
 ...
 str表示是高斯金字塔还是DOG金字塔
*/
void Display::mosaic_pyramid(const std::vector<std::vector<cv::Mat>> &pyramid, cv::Mat &pyramid_image, int nOctaceLayers, std::string str)
{
  // 获得每组金字塔图像大小
  std::vector<std::vector<int>> temp_size;
  ...
  for (auto beg = temp_size.begin(); beg != temp_size.end(); ++beg)
  {
    ...
    {
      if (str == std::string("高斯金字塔"))
      {
        total_cols = (nOctaceLayers + 3) * ((*beg)[1]); // 获取列大小
      }
      else if (str == std::string("DOG金字塔"))
      {
  ...
  for (auto beg = pyramid.cbegin(); beg != pyramid.cend(); ++beg, ++i)
  {
    ...
    for (auto it = (*beg).cbegin(); it != (*beg).cend(); ++it)
    {
      ...
      cv::Mat temp(pyramid_image, cv::Rect(accumulate_col - temp_size[i][1], accumulate_row - temp_size[i][0], it->cols, it->rows));
      cv::Mat temp_it;
      normalize(*it, temp_it, 0, 255, cv::NORM_MINMAX, CV_32FC1);
 ...
 */
void Display::write_mosaic_pyramid(const std::vector<std::vector<cv::Mat>> &gauss_pyr_1, const std::vector<std::vector<cv::Mat>> &dog_pyr_1,
                                   const std::vector<std::vector<cv::Mat>> &gauss_pyr_2, const std::vector<std::vector<cv::Mat>> &dog_pyr_2, int nOctaveLayers)
{
  // 显示参考和待配准高斯金字塔图像
  cv::Mat gauss_image_1, gauss_image_2;
  mosaic_pyramid(gauss_pyr_1, gauss_image_1, nOctaveLayers, std::string("高斯金字塔"));
  mosaic_pyramid(gauss_pyr_2, gauss_image_2, nOctaveLayers, std::string("高斯金字塔"));
  imwrite("../image_save/参考图像高斯金字塔.jpg", gauss_image_1);
  imwrite("../image_save/待配准图像高斯金字塔.jpg", gauss_image_2);

  // 显示参考和待配准DOG金字塔图像
  cv::Mat dog_image_1, dog_image_2;
  mosaic_pyramid(dog_pyr_1, dog_image_1, nOctaveLayers, std::string("DOG金字塔"));
  mosaic_pyramid(dog_pyr_2, dog_image_2, nOctaveLayers, std::string("DOG金字塔"));
