...
#include <opencv2/core.hpp>       //opencv基本数据结构
...
/******************根据输入图像大小计算高斯金字塔的组数****************************/
/*image 表示原始输入灰度图像,inline函数必须在声明处定义
double_size_image 表示是否在构建金字塔之前上采样原始图像
*/
int Sift::num_octaves(const cv::Mat &image)
{
  int temp;
  float size_temp = (float)cv::min(image.rows, image.cols);
  temp            = cvRound(log(size_temp) / log((float)2) - 2); // 用于计算一个数的自然对数
  ...
  if (temp > MAX_OCTAVES) // 尺度空间最大组数设置为 MAX_OCTAVES=8
  {
    temp = MAX_OCTAVES;
  }
...
void sobelfilter(cv::Mat &image, cv::Mat &G)
{
  ...
  G = cv::Mat::zeros(rows, cols, CV_32FC1); // 在每个像素点处的灰度大小由Gx和Gy共同决定
  ...
  for (int i = 0; i < rows; i++)
  {
    for (int j = 0; j < cols; j++)
    {
      ...
      {
        float dx = image.at<float>(i - 1, j + 1) - image.at<float>(i - 1, j - 1) + 2 * image.at<float>(i, j + 1) - 2 * image.at<float>(i, j - 1) + image.at<float>(i + 1, j + 1) - image.at<float>(i + 1, j - 1);

        float dy = image.at<float>(i + 1, j - 1) - image.at<float>(i - 1, j - 1) + 2 * image.at<float>(i + 1, j) - 2 * image.at<float>(i - 1, j) + image.at<float>(i + 1, j + 1) - image.at<float>(i - 1, j + 1);
...
/*size表示核半径，因此核宽度是2*size+1
 ...
 kernel表示生成的滤波核
 */
static void roewa_kernel(int size, float scale, cv::Mat &kernel)
{
...
/************************创建高斯金字塔第一组，第一层图像************************************/
/* image 表示输入原始图像
 init_image 表示生成的高斯尺度空间的第一层图像
 */
void Sift::create_initial_image(const cv::Mat &image, cv::Mat &init_image)
{
  cv::Mat gray_image;
  ...
  cv::Mat floatImage; // 转换到0-1之间的浮点类型数据归一化，方便接下来的处理

  gray_image.convertTo(floatImage, CV_32FC1, 1.0 / 255.0, 0);

  double sig_diff = 0; // 高斯平滑的标准差
  ...
  if (double_size)
  {
    ...
    // 通过线性插值的方法改变图像尺寸的大小
    resize(floatImage, temp_image, cv::Size(2 * floatImage.cols, 2 * floatImage.rows), 0.0, 0.0, cv::INTER_LINEAR);
...
// 目的是为了减少冗余特征点
void Sift::sobel_create_initial_image(const cv::Mat &image, cv::Mat &init_image)
{
  cv::Mat gray_image, gray_images; // gray_images用于存放经过sobel算子操作后的图像
...
/**************************生成高斯金字塔*****************************************/
/*init_image 表示已经生成的高斯金字塔第一层图像
 gauss_pyramid 表示生成的高斯金字塔
 nOctaves 表示高斯金字塔的组数
*/
void Sift::build_gaussian_pyramid(const cv::Mat &init_image, std::vector<std::vector<cv::Mat>> &gauss_pyramid, int nOctaves)
{
  ...
  for (int i = 0; i < nOctaves; ++i) // 对于每一组
  {
    for (int j = 0; j < nOctaveLayers + 3; ++j) // 对于组内的每一层
    {
      ...
      else if (j == 0) // 对于每组的第一层
      {
        /* 从上一个octave的最后一层图像生成当前octave的第一层图像。通过对图像进行下采样（减少图像尺寸），
           该操作为构建高斯金字塔的下一个octave提供了起始图像。*/
        //  上一组最后一层，当前组第一层，尺寸减半，被忽略的项，线性插值
        resize(gauss_pyramid[i - 1][3], gauss_pyramid[i][0],
               cv::Size(gauss_pyramid[i - 1][3].cols / 2, gauss_pyramid[i - 1][3].rows / 2),
               0, 0, cv::INTER_LINEAR);
      }
      else
      {
        ...
        // 通过在高斯金字塔的一个octave内逐层应用不同程度的高斯模糊，来创建一系列在尺度上逐渐变化的图像
        GaussianBlur(gauss_pyramid[i][j - 1], gauss_pyramid[i][j], kernel_size, sig[j], sig[j]);
      }
    } // endfor: every layer in current octave

  } // endfor: every octave
}

/*******************生成高斯差分金字塔，即LOG金字塔*************************/
/*dog_pyramid 表示DOG金字塔
 gauss_pyramin 表示高斯金字塔*/
void Sift::build_dog_pyramid(std::vector<std::vector<cv::Mat>> &dog_pyramid, const std::vector<std::vector<cv::Mat>> &gauss_pyramid)
{
  std::vector<std::vector<cv::Mat>>::size_type nOctaves = gauss_pyramid.size();

  for (std::vector<std::vector<cv::Mat>>::size_type i = 0; i < nOctaves; ++i)
  {
    // 用于存放每一个梯度中的所有尺度层
    ...
    {
      cv::Mat temp_img = gauss_pyramid[i][j + 1] - gauss_pyramid[i][j];
...
void Sift::amplit_orient(const cv::Mat &image, std::vector<cv::Mat> &amplit, std::vector<cv::Mat> &orient, float scale, int nums)
{
  ...
  cv::Mat kernel; // kernel(2 * radius + 1, 2 * radius + 1, CV_32FC1);

  roewa_kernel(radius, scale, kernel); // 返回滤波核，也即指数部分,存放在矩阵的右下角

  // 四个滤波模板生成
  cv::Mat W34 = cv::Mat::zeros(2 * radius + 1, 2 * radius + 1, CV_32FC1); // 把kernel矩阵下半部分复制到对应部分
  cv::Mat W12 = cv::Mat::zeros(2 * radius + 1, 2 * radius + 1, CV_32FC1); // 把kernel矩阵上半部分复制到对应部分
  cv::Mat W14 = cv::Mat::zeros(2 * radius + 1, 2 * radius + 1, CV_32FC1); // 把kernel矩阵右半部分复制到对应部分
  cv::Mat W23 = cv::Mat::zeros(2 * radius + 1, 2 * radius + 1, CV_32FC1); // 把kernel矩阵左半部分复制到对应部分

  kernel(cv::Range(radius + 1, 2 * radius + 1), cv::Range::all()).copyTo(W34(cv::Range(radius + 1, 2 * radius + 1), cv::Range::all()));
  kernel(cv::Range(0, radius), cv::Range::all()).copyTo(W12(cv::Range(0, radius), cv::Range::all()));
  kernel(cv::Range::all(), cv::Range(radius + 1, 2 * radius + 1)).copyTo(W14(cv::Range::all(), cv::Range(radius + 1, 2 * radius + 1)));
  kernel(cv::Range::all(), cv::Range(0, radius)).copyTo(W23(cv::Range::all(), cv::Range(0, radius)));

  // 滤波
  cv::Mat M34, M12, M14, M23;
  double eps = 0.00001;

  // float_image为图像归一化后的图像数据，做卷积运算
  filter2D(image, M34, CV_32FC1, W34, cv::Point(-1, -1), eps);
  filter2D(image, M12, CV_32FC1, W12, cv::Point(-1, -1), eps);
  filter2D(image, M14, CV_32FC1, W14, cv::Point(-1, -1), eps);
  filter2D(image, M23, CV_32FC1, W23, cv::Point(-1, -1), eps);
...
inline void adjustIDs(int &id, const int &range)
{
  if (id < 0)
  {
    id = id + range;
...
static float clac_orientation_hist(const cv::Mat &image, cv::Point pt, float scale, int n, float *hist)
{
  ...
  // 使用 AutoBuffer 分配一段内存，这里多出4个空间的目的是为了方便后面平滑直方图的需要
  ...
  for (int i = -radius; i < radius; ++i)
  {
    ...
    for (int j = -radius; j < radius; ++j)
    {
      ...
      X[k] = dx;
      Y[k] = dy;
      W[k] = (i * i + j * j) * exp_scale; // more distant, more weight

      ++k;
    }
  } // endfor: 已经得到邻域像素的水平差分和竖直差分

  len = k; // 邻域内特征点的个数

  /* Hardware Abstraction Laye. OpenCV的硬件加速层提供了一组低级的函数，这些函数被设计来直接利用底层硬件的特性，以提高特定计算任务的性能。*/
  // 对输入数组中每个元素执行自然指数函数的操作
  cv::hal::exp(W, W, len);                  // 计算邻域内所有像素的高斯权重
  cv::hal::fastAtan2(Y, X, Ori, len, true); // 计算邻域内所有像素的梯度方向，角度范围0-360度
  /*用于计算两个浮点数组（通常表示为向量的X和Y坐标）的每个元素对应的欧几里得距离或者说向量的幅度。函数接受两个浮点数组：
    第一个数组包含X坐标（或水平分量），第二个数组包含Y坐标（或垂直分量）。输出是一个浮点数组，其中每个元素是输入坐标对的向量幅度. */
  ...
  for (int i = 0; i < n; ++i)
  {
    hist[i] = (temp_hist[i - 2] + temp_hist[i + 2]) * (1.f / 16.f) + (temp_hist[i - 1] + temp_hist[i + 1]) * (4.f / 16.f) + temp_hist[i] * (6.f / 16.f);
  }
...
static float clac_orientation_hist_2(cv::Mat &image, cv::Point pt, float scale, int n, float *hist)
{
  cv::Mat output_image; // 使用 sobel 滤波器计算的图像的梯度幅度图像
  ...
  for (int i = -radius; i < radius; ++i)
  {
    ...
    for (int j = -radius; j < radius; ++j)
    {
      ...
      float dx = output_image.at<float>(y - 1, x + 1) - output_image.at<float>(y - 1, x - 1) + 2 * output_image.at<float>(y, x + 1) - 2 * output_image.at<float>(y, x - 1) + output_image.at<float>(y + 1, x + 1) - output_image.at<float>(y + 1, x - 1);

      float dy = output_image.at<float>(y + 1, x - 1) - output_image.at<float>(y - 1, x - 1) + 2 * output_image.at<float>(y + 1, x) - 2 * output_image.at<float>(y - 1, x) + output_image.at<float>(y + 1, x + 1) - output_image.at<float>(y - 1, x + 1);
...
/*amplit表示特征点所在层的梯度幅度，即输入图像对应像素点的梯度存在了对应位置
  ...
  n表示主方向直方图bin个数
 */
static float calc_orient_hist(const cv::Mat &amplit, const cv::Mat &orient, cv::Point pt, float scale, float *hist, int n)
{
  ...
  radius = cv::min(radius, cv::min(num_row / 2, num_col / 2));
  ...
  // 矩形区域的边界，计算高斯权值
  cv::Range x_rng(-(point.x - radius_x_left), radius_x_right - point.x);
  cv::Range y_rng(-(point.y - radius_y_up), radius_y_down - point.y);
  ...
  // 索引特征点周围的像素梯度幅度，梯度方向
  cv::Mat sub_amplit = amplit(cv::Range(radius_y_up, radius_y_down + 1), cv::Range(radius_x_left, radius_x_right + 1));
  cv::Mat sub_orient = orient(cv::Range(radius_y_up, radius_y_down + 1), cv::Range(radius_x_left, radius_x_right + 1));

  // cv::Mat W = sub_amplit.mul(gauss_weight);			//加入高斯权重，计算高斯权重时，正确匹配点对反而变少了
  cv::Mat W = sub_amplit; // 没加高斯权重，梯度幅值

  // 计算直方图
  cv::AutoBuffer<float> buffer(n + 4);
...
static bool adjust_local_extrema_1(const std::vector<std::vector<cv::Mat>> &dog_pyr, cv::KeyPoint &kpt, int octave, int &layer, int &row,
                                   int &col, int nOctaveLayers, float contrastThreshold, float edgeThreshold, float sigma)
{
  ...
  {
    const cv::Mat &img  = dog_pyr[octave][layer];     // 当前层图像索引
    const cv::Mat &prev = dog_pyr[octave][layer - 1]; // 之前层图像索引
    const cv::Mat &next = dog_pyr[octave][layer + 1]; // 下一层图像索引
    ...
    // 计算特征点周围混合二阶偏导数
    float dxy = (img.at<float>(row + 1, col + 1) + img.at<float>(row - 1, col - 1) - img.at<float>(row + 1, col - 1) - img.at<float>(row - 1, col + 1)) * (1.f / 4.f);
    float dxz = (next.at<float>(row, col + 1) + prev.at<float>(row, col - 1) - next.at<float>(row, col - 1) - prev.at<float>(row, col + 1)) * (1.f / 4.f);
    float dyz = (next.at<float>(row + 1, col) + prev.at<float>(row - 1, col) - next.at<float>(row - 1, col) - prev.at<float>(row + 1, col)) * (1.f / 4.f);
    ...
    if (layer < 1 || layer > nOctaveLayers || col < IMG_BORDER || col > img.cols - IMG_BORDER || row < IMG_BORDER || row > img.rows - IMG_BORDER)
    {
      return false;
    }
  }
  ...
  // 高对比度的特征对图像的变形是稳定的
  {
    const cv::Mat &img  = dog_pyr[octave][layer];
    const cv::Mat &prev = dog_pyr[octave][layer - 1];
    const cv::Mat &next = dog_pyr[octave][layer + 1];
...
static bool adjust_local_extrema_2(const std::vector<std::vector<cv::Mat>> &dog_pyr, cv::KeyPoint &kpt, int octave, int &layer, int &row,
                                   int &col, int nOctaveLayers, float contrastThreshold, float edgeThreshold, float sigma)
{
  ...
  for (; i < MAX_INTERP_STEPS; ++i) // 最大迭代次数
  {
    ...
    // 计算特征点周围混合二阶偏导数
    float dxy = (img.at<float>(row + 1, col + 1) + img.at<float>(row - 1, col - 1) - img.at<float>(row + 1, col - 1) - img.at<float>(row - 1, col + 1)) * cross_deriv_scale;
    float dxz = (next.at<float>(row, col + 1) + prev.at<float>(row, col - 1) - next.at<float>(row, col - 1) - prev.at<float>(row, col + 1)) * cross_deriv_scale;
    float dyz = (next.at<float>(row + 1, col) + prev.at<float>(row - 1, col) - next.at<float>(row - 1, col) - prev.at<float>(row + 1, col)) * cross_deriv_scale;

    cv::Matx33f H(dxx, dxy, dxz, dxy, dyy, dyz, dxz, dyz, dzz);

    cv::Vec3f dD(dx, dy, dz);

    cv::Vec3f X = H.solve(dD, cv::DECOMP_SVD); // delta_d=Hx
  ...
  } // endfor: 已经找到特征点准确位置 或者 没有找到特征点准确位置
  ...
  /******************************删除边缘响应比较强的点************************************/
  /* 虽然边缘可以产生强烈的图像梯度响应，但它们的位置却可能因视角变化、光照条件变化或其他因素而发生显著变化，从而降低特征匹配的准确性和稳定性。
    1. 位置不稳定：边缘上的点因为仅在边缘的法线方向上有强烈变化，而在边缘本身的方向上变化较小，
       这导致其在边缘方向的位置可能因小幅度的视角变化或光照变化而产生较大的偏移。
    2. 匹配不可靠：边缘上的特征点可能在不同的图像中表现出不同的特性，尤其是在边缘强度或方向发生变化时，这使得基于这些点的特征匹配变得不可靠。
    3. 抗噪声能力弱：边缘响应较强的点容易受到图像噪声的影响，尤其是在边缘附近，噪声可能会被错误地解释为特征点。
  */
  //  再次计算特征点位置二阶偏导数，获取特征点出的 Hessian 矩阵，主曲率通过 2X2 的 Hessian 矩阵求出
  //  一个定义不好的高斯差分算子的极值在横跨边缘的地方有较大的主曲率而在垂直边缘的方向有较小的主曲率，对应原因1
  float v2  = (float)img.at<float>(row, col) * 2.f;
  float dxx = (img.at<float>(row, col + 1) + img.at<float>(row, col - 1) - v2) * second_deriv_scale;
  float dyy = (img.at<float>(row + 1, col) + img.at<float>(row - 1, col) - v2) * second_deriv_scale;
  float dxy = (img.at<float>(row + 1, col + 1) + img.at<float>(row - 1, col - 1) - img.at<float>(row + 1, col - 1) - img.at<float>(row - 1, col + 1)) * cross_deriv_scale;
...
/*dog_pyr表示高斯金字塔			原始 SIFT 算子
 gauss_pyr表示高斯金字塔
 keypoints表示检测到的特征点*/
void Sift::find_scale_space_extrema(const std::vector<std::vector<cv::Mat>> &dog_pyr, const std::vector<std::vector<cv::Mat>> &gauss_pyr,
                                    std::vector<cv::KeyPoint> &keypoints)
{
  ...
  for (int i = 0; i < nOctaves; ++i) // 对于每一组
  {
    ...
    {
      const cv::Mat &curr_img = dog_pyr[i][j];     // 当前层
      const cv::Mat &prev_img = dog_pyr[i][j - 1]; // 上一层
      const cv::Mat &next_img = dog_pyr[i][j + 1]; // 下一层
      ...
      for (int r = IMG_BORDER; r < num_row - IMG_BORDER; ++r)
      {
        ...
        for (int c = IMG_BORDER; c < num_col - IMG_BORDER; ++c)
        {
          ...
          // 开始检测特征点
          if (abs(val) > threshold && isExtreme(val, curr_ptr, prev_ptr, next_ptr, c, step))
          {
            //++numKeys;
            ...
            // max_hist值对应的方向为主方向
            float max_hist = clac_orientation_hist(gauss_pyr[octave][layer], cv::Point(c1, r1), scale, n, hist);
...
/************该函数在DOG金字塔上进行特征点检测，特征点精确定位，删除低对比度点，删除边缘响应较大点**********/
// 对特征点进行方向的细化 + 增加更多的主方向版本 —— 此处细化是对最后要给关键点进行赋值时的细化，还可以考虑直接对方向直方图进行细化
void Sift::find_scale_space_extrema1(const std::vector<std::vector<cv::Mat>> &dog_pyr, std::vector<std::vector<cv::Mat>> &gauss_pyr,
                                     std::vector<cv::KeyPoint> &keypoints)
{
  int nOctaves = (int)dog_pyr.size(); // 子八度个数
  ...
  std::vector<cv::Mat> amplit; // 存放高斯差分金字塔每一层的梯度幅度图像
  std::vector<cv::Mat> orient; // 存放高斯差分金字塔每一层的梯度方向图像
...
/*x_range表示x方向的范围，X表示生成的沿x轴变化的网格
y_range表示y方向的范围，Y表示生成沿y轴变换的网格
*/
static void meshgrid(const cv::Range &x_range, const cv::Range &y_range, cv::Mat &X, cv::Mat &Y)
{
...
/******************************计算一个特征点的描述子***********************************/
/*gauss_image 表示特征点所在的高斯层
  main_angle 表示特征点的主方向，角度范围是0-360度
  pt 表示特征点在高斯图像上的坐标，相对与本组，不是相对于最底层
  scale 表示特征点所在层的尺度，相对于本组，不是相对于最底层
  d 表示特征点邻域网格宽度
  n 表示每个网格内像素梯度角度等分个数
  descriptor 表示生成的特征点的描述子*/
static void calc_sift_descriptor(const cv::Mat &gauss_image, float main_angle, cv::Point2f pt,
                                 float scale, int d, int n, float *descriptor)
{
  ...
  // 特征点邻域半径
  radius = cv::min(radius, (int)sqrt((double)rows * rows + cols * cols));
  ...
  cv::AutoBuffer<float> buf(6 * len + histlen);
  ...
  for (int i = -radius; i < radius; ++i) // i 对应 y 行坐标
  {
    ...
    {
      // 对于关键点周围的每个采样点，将其坐标旋转到以关键点主方向为基准的坐标系中
      ...
      // 这里rbin，cbin范围是(-1,d)；确保旋转后的采样点必须落在预定义的网格内；采样点的原始图像坐标也不能超出图像边界
  ...
  for (k = 0; k < len; ++k)
  {
    ...
    // 调整后的采样点梯度方向，通过减去关键点的主方向main_angle来实现旋转不变性。
    float temp = Angle[k] - main_angle;

    /*if (temp > 180 && temp < 360)
      temp = 360 - temp;*/

    float obin = temp * bins_per_rad; // 采样点梯度方向在方向直方图中的位置

    float mag = Mag[k] * W[k]; // 子区域内像素点乘以权值后的梯度幅值

    // 采样点在子区域网格和方向直方图中的整数索引
    int r0 = cvFloor(rbin); // r0取值集合是{-1，0，1，2，3}，position
    int c0 = cvFloor(cbin); // c0取值集合是{-1，0，1，2，3}
    int o0 = cvFloor(obin); // orientation
    ...
    // 该像素所在网格的索引;当前采样点对应的直方图bin的起始索引
  ...
  for (int i = 0; i < lenght; ++i)
  {
    descriptor[i] = cv::min(descriptor[i], DESCR_MAG_THR);
  }
                                 ...
                                 float main_ori, int d, int n, float *ptr_des)
{
  ...
  radius       = cv::min(radius, cv::min(num_rows / 2, num_cols / 2)); // 特征点邻域半径
  ...
  cv::Mat c_rot = X * cos_t - Y * sin_t;
  cv::Mat r_rot = X * sin_t + Y * cos_t;
  cv::Mat GLOH_angle, GLOH_amplit;
  ...
  for (int i = 0; i < len; ++i)
  {
    hist[i] = cv::min(hist[i], DESCR_MAG_THR);
  }
...
/******************************计算一个特征点的描述子—改进版***********************************/
static void improve_calc_sift_descriptor(const cv::Mat &gauss_image, float main_angle, cv::Point2f pt,
                                         float scale, int d, int n, float *descriptor)
{
  int n1 = 16, n2 = 6, n3 = 4;
  ...
  std::vector<int> v; // 存放外邻域像素点对应的序号
...
void Sift::calc_sift_descriptors(const std::vector<std::vector<cv::Mat>> &gauss_pyr, std::vector<cv::KeyPoint> &keypoints,
                                 cv::Mat &descriptors, const std::vector<cv::Mat> &amplit, const std::vector<cv::Mat> &orient)
{
  ...
  for (size_t i = 0; i < keypoints.size(); ++i) // 对于每一个特征点
  {
    ...
    // 得到特征点相对于本组的坐标，不是最底层
    cv::Point2f pt(keypoints[i].pt.x / (1 << octaves), keypoints[i].pt.y / (1 << octaves));
...
/*image表示输入的原始图像
 ...
 orient表示尺度空间像素的梯度方向
 */
void Sift::build_sar_sift_space(const cv::Mat &image, std::vector<cv::Mat> &sar_harris_fun, std::vector<cv::Mat> &amplit, std::vector<cv::Mat> &orient)
{
  // 转换输入图像格式
  cv::Mat gray_image;
  if (image.channels() != 1)
  ...
  for (int i = 0; i < Mmax; ++i)
  {
    ...
    // 四个滤波模板生成
    cv::Mat W34 = cv::Mat::zeros(2 * radius + 1, 2 * radius + 1, CV_32FC1);
    cv::Mat W12 = cv::Mat::zeros(2 * radius + 1, 2 * radius + 1, CV_32FC1);
    cv::Mat W14 = cv::Mat::zeros(2 * radius + 1, 2 * radius + 1, CV_32FC1);
    cv::Mat W23 = cv::Mat::zeros(2 * radius + 1, 2 * radius + 1, CV_32FC1);
    ...
    cv::Mat M34, M12, M14, M23;
    double eps = 0.00001;
    filter2D(float_image, M34, CV_32FC1, W34, cv::Point(-1, -1), eps);
    filter2D(float_image, M12, CV_32FC1, W12, cv::Point(-1, -1), eps);
    filter2D(float_image, M14, CV_32FC1, W14, cv::Point(-1, -1), eps);
    filter2D(float_image, M23, CV_32FC1, W23, cv::Point(-1, -1), eps);
    ...
    // 构建sar-Harris矩阵
    // cv::Mat Csh_11 = log(scale)*log(scale)*Gx.mul(Gx);
    // cv::Mat Csh_12 = log(scale)*log(scale)*Gx.mul(Gy);
    // cv::Mat Csh_22 = log(scale)*log(scale)*Gy.mul(Gy);

    cv::Mat Csh_11 = scale * scale * Gx.mul(Gx);
    cv::Mat Csh_12 = scale * scale * Gx.mul(Gy);
    cv::Mat Csh_22 = scale * scale * Gy.mul(Gy); // 此时阈值为0.8

    // cv::Mat Csh_11 = Gx.mul(Gx);
    // cv::Mat Csh_12 = Gx.mul(Gy);
    // cv::Mat Csh_22 = Gy.mul(Gy);//此时阈值为0.8/100

    // 高斯权重
    float gauss_sigma = sqrt(2.f) * scale;
    int size          = cvRound(3 * gauss_sigma);

    cv::Size kern_size(2 * size + 1, 2 * size + 1);
    ...
    cv::Mat Csh_21 = Csh_12;

    // 构建sar_harris函数
    cv::Mat temp_add = Csh_11 + Csh_22;
 ...
 */
void Sift::calc_gloh_descriptors(const std::vector<cv::Mat> &amplit, const std::vector<cv::Mat> &orient, const std::vector<cv::KeyPoint> &keys, cv::Mat &descriptors)
{
  ...
  for (int i = 0; i < num_keys; ++i)
  {
    ...
    // 得到特征点相对于本组的坐标，不是最底层
    cv::Point2f point(keys[i].pt.x / (1 << octaves), keys[i].pt.y / (1 << octaves));

    std::cout << "layer=" << octaves << std::endl;
...
/*image表示输入的图像
  ...
  keypoints表示检测到的特征点
  std::vector<float>& cell_contrast 用于存放一个单元格中所有特征点的对比度
  std::vector<float>& cell_contrasts用于存放一个尺度层中所有单元格中特征点的对比度
  std::vector<std::vector<std::vector<float>>>& all_cell_contrasts用于存放所有尺度层中所有单元格的对比度
  std::vector<std::vector<float>>& average_contrast用于存放所有尺度层中多有单元格的平均对比度*/
void Sift::detect(const cv::Mat &image,
                  std::vector<std::vector<cv::Mat>> &gauss_pyr, std::vector<std::vector<cv::Mat>> &dog_pyr,
                  std::vector<cv::KeyPoint> &keypoints,
                  std::vector<std::vector<std::vector<float>>> &all_cell_contrasts, std::vector<std::vector<float>> &average_contrast,
                  std::vector<std::vector<int>> &n_cells, std::vector<int> &num_cell,
                  std::vector<std::vector<int>> &available_n, std::vector<int> &available_num,
                  std::vector<cv::KeyPoint> &final_keypoints, std::vector<cv::KeyPoint> &Final_keypoints, std::vector<cv::KeyPoint> &Final_Keypoints)
{
...
void Sift::comput_des(const std::vector<std::vector<cv::Mat>> &gauss_pyr, std::vector<cv::KeyPoint> &final_keypoints, const std::vector<cv::Mat> &amplit,
